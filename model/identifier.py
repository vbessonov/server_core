...
class Identifier(Base, IdentifierConstants):
    ...
    def __repr__(self):
        ...
        return native_string(
            "%s/%s ID=%s%s" % (self.type, self.identifier, self.id, title)
        )
    ...
    @property
    def urn(self):
        identifier_text = urllib.parse.quote(self.identifier)
        if self.type == Identifier.ISBN:
            ...
            identifier_type = urllib.parse.quote(self.type)
    ...
    def type_and_identifier_for_urn(cls, identifier_string):
        ...
        if m:
            ...
            type, identifier_string = list(map(
                urllib.parse.unquote, identifier_string.split("/", 1)))
            ...
            identifier_string = urllib.parse.unquote(identifier_string)
